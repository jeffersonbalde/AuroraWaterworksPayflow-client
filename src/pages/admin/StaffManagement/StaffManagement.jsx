// src/pages/admin_staff/StaffManagement.jsx - UPDATED WITH CONSISTENT BUTTON STYLING
...
const StaffManagement = () => {
  ...
  useEffect(() => {
    filterAndSortStaff();
  }, [staffUsers, searchTerm, filterStatus, sortField, sortDirection]);
  ...
  const fetchStaffUsers = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Filter only staff users (exclude admin and clients)
        const staffUsers =
          data.users?.filter((user) => user.role === "staff") || [];
  ...
  const filterAndSortStaff = () => {
    ...
    filtered.sort((a, b) => {
      ...
      if (sortField === "created_at" || sortField === "approved_at") {
  ...
  // FIXED Staff Avatar Component
  const StaffAvatar = ({ staff, size = 40 }) => {
  ...
  const handleStaffSave = (savedStaff) => {
    if (editingStaff) {
      ...
      setStaffUsers((prev) => [...prev, savedStaff]);
    }
  ...
  const handleDeactivateConfirm = async (deactivateReason) => {
    ...
    showProcessingAlert("Deactivating Staff", "Please wait while we update the account status.");
    ...
    try {
      ...
      if (response.ok) {
        ...
        setStaffUsers((prev) =>
          prev.map((s) =>
            s.id === selectedStaff.id ? { ...s, status: "inactive" } : s
          )
        );
      ...
      showAlert.error(
        "Deactivation Failed",
        error.message || "Failed to deactivate staff"
      );
    } finally {
      showAlert.close();
  ...
  const handleActivate = async (staff) => {
    ...
    showProcessingAlert("Activating Staff", "Please wait while we update the account status.");

    try {
      const response = await fetch(
        `${import.meta.env.VITE_LARAVEL_API}/admin/users/${staff.id}/activate`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        showToast.success("Staff activated successfully!");
        setStaffUsers((prev) =>
      ...
      showAlert.error(
        "Activation Failed",
        error.message || "Failed to activate staff"
      );
    } finally {
      showAlert.close();
      setActionLoading(null);
  ...
  const handleDeleteStaff = async (staff) => {
    ...
    const confirmation = await showAlert.confirm(
      ...
      `Are you sure you want to permanently delete ${staff.name}'s account?`,
      "Yes, Delete",
      "Cancel",
      "warning"
    );
    ...
    setActionLock(true);
    setActionLoading(staff.id);
    showProcessingAlert("Deleting Staff Account", "Please wait while we remove this user.");
  ...
  const isActionDisabled = (staffId = null) => {
    return actionLock || (actionLoading && actionLoading !== staffId);
  };
  ...
  // FIXED: Improved Skeleton Loader for Table Rows
    ...
    <tr className="align-middle" style={{ height: "70px" }}>
      ...
      <td>
        <div className="d-flex align-items-center">
          {/* Avatar skeleton */}
          <div className="placeholder-wave me-3">
            <span
  ...
  // FIXED: Improved Skeleton Loader for Statistics Cards
  ...
  return (
    <div className="container-fluid px-3 py-2 staff-management-container fadeIn">
      {/* Page Header */}
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3">
        <div className="flex-grow-1 mb-2 mb-md-0">
          ...
          <p className="mb-0 small" style={{ color: "var(--text-muted)" }}>
            Manage staff member accounts and access permissions
          </p>
        </div>
        <div className="d-flex align-items-center gap-2 flex-wrap">
          ...
          >
            ...
            Total Staff: {loading ? "..." : staffUsers.length}
          </div>
          <button
                ...
                <div className="d-flex align-items-center">
                  <div className="flex-grow-1">
                    ...
                    <div
                      ...
                      style={{ color: "var(--danger-color)" }}
                    >
                      {staffUsers.filter((s) => s.status === "inactive").length}
                    </div>
                  </div>
                  <div className="col-auto">
                    <i
                        ...
                        <td className="text-center">
                          <div className="d-flex justify-content-center gap-1">
                            <button
                              ...
                              disabled={isActionDisabled(staff.id)}
                              title="View Details"
                              style={{
                                width: "36px",
                                height: "36px",
      ...
      {showDeactivateModal && selectedStaff && (
        <DeactivateModal
          ...
          loading={actionLoading === selectedStaff.id}
        />
      )}
    </div>
  );
