// src/pages/admin_staff/BillingManagement/BillingManagement.jsx
...
const BillingManagement = () => {
  ...
  // Combined loading state - show skeleton until both bills and customers are loaded
  ...
  const [restatingBill, setRestatingBill] = useState(null);
  ...
  const filterAndSortBills = () => {
    ...
    if (searchTerm.trim()) {
      ...
      filtered = filtered.filter((bill) => {
        const customer = customers.find((c) => c.id === bill.user_id);
  ...
  // Bills are created automatically when readings are input in MeterReading component
  // This component is for viewing and managing existing bills only
  ...
  const handleRestateAmount = (bill) => {
    if (actionLock) {
      showToast.warning("Please wait until the current action completes");
      return;
    }
    ...
    const defaultAmount =
      bill.restated_amount ?? bill.total_payable ?? bill.amount ?? 0;

    setRestatingBill(bill);
    setRestateForm({
  ...
  const handleRestateSubmit = async (event) => {
    ...
    setActionLock(true);
    setActionLoading(restatingBill.id);
    ...
    try {
      ...
      if (response.ok) {
        const formattedBill = {
          ...data.bill,
          total_payable: parseFloat(data.bill.total_payable) || 0,
          restated_amount: data.bill.restated_amount
            ? parseFloat(data.bill.restated_amount)
            : null,
        };
        setBills((prev) =>
          prev.map((bill) =>
            bill.id === restatingBill.id ? formattedBill : bill
          )
        );
  ...
  const handleMarkAsPaid = async (bill) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_LARAVEL_API}/admin/bills/${bill.id}/mark-paid`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      showAlert.error(
        "Update Failed",
        error.message || "Failed to mark bill as paid"
      );
    } finally {
      setActionLoading(null);
      setActionLock(false);
  ...
  const isActionDisabled = (billId = null) => {
    return actionLock || (actionLoading && actionLoading !== billId);
  };
  ...
  const totalRevenue = bills
    ...
    .reduce((sum, bill) => sum + bill.total_payable, 0);
  ...
  return (
    <div className="container-fluid px-3 py-2 billing-management-container fadeIn">
      {/* Page Header */}
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3">
        <div className="flex-grow-1 mb-2 mb-md-0">
          ...
          <p className="mb-0 small" style={{ color: "var(--text-muted)" }}>
            View and manage customer bills. Bills are created automatically when
            readings are input in Meter Reading.
          </p>
        </div>
        <div className="d-flex align-items-center gap-2 flex-wrap">
          ...
          >
            ...
            Total Bills: {isLoading ? "..." : bills.length}
          </div>
          <button
            ...
            <div className="card stats-card h-100">
              <div className="card-body p-3">
                <div className="d-flex align-items-center">
                  ...
                  <div className="col-auto">
                    <i
                      className="fas fa-file-invoice-dollar fa-2x"
            ...
            <div className="table-responsive">
              ...
              <div className="text-center py-4">
                ...
                <span className="small" style={{ color: "var(--text-muted)" }}>
                  {loading ? "Fetching bills..." : "Loading customer data..."}
                </span>
              </div>
            </div>
                ...
                <table className="table table-striped table-hover mb-0">
                  ...
                  <tbody>
                    {currentBills.map((bill, index) => {
                      const customer = getCustomerInfo(bill.user_id);
                      return (
                          ...
                          <td className="text-center">
                            <div className="d-flex justify-content-center gap-1">
                              <button
                                ...
                                disabled={isActionDisabled(bill.id)}
                                title="View Details"
                                style={{
                                  width: "36px",
                                  height: "36px",
                              ...
                              <button
                                className="btn btn-sm text-white"
                                onClick={() => handleRestateAmount(bill)}
                                disabled={isActionDisabled(bill.id)}
                          ...
                          <td>
                            <div style={{ color: "var(--text-primary)" }}>
                              ...
                              <div
                                ...
                                style={{ color: "var(--text-muted)" }}
                              >
                                WWS: {customer.wws_id || bill.wws_id}
                              </div>
                            </div>
                          </td>
                          ...
                          <td>
                            <div style={{ color: "var(--text-primary)" }}>
                              <div className="fw-medium text-success">
                                ₱{bill.total_payable?.toFixed(2) || "0.00"}
                              </div>
                              {bill.restated_amount && (
                                <div className="small text-warning">
                                  Restated: ₱{bill.restated_amount.toFixed(2)}
                                </div>
                              )}
                              {bill.penalty > 0 && (
                                <div className="small text-danger">
                                  Penalty: ₱{bill.penalty.toFixed(2)}
                                </div>
                              )}
                            </div>
                          </td>
                          ...
                          <td>
                            <div style={{ color: "var(--text-primary)" }}>
                              <div className="small fw-medium">
                                {bill.meter_reader || "Not specified"}
                              </div>
                              {bill.qr_number && (
                                <div
                                  className="small"
      ...
      {showDetailsModal && selectedBill && (
        <BillDetailsModal
          bill={selectedBill}
          customer={getCustomerInfo(selectedBill.user_id)}
          onClose={() => {
      ...
      {showRestateModal && restatingBill && (
        <Portal>
          <div
                  ...
                  >
                    <div>
                      ...
                      <small className="text-white-50">
                        Bill #{restatingBill.id} •{" "}
                        {getCustomerInfo(restatingBill.user_id).name}
                      </small>
                    </div>
                    <button
                      type="button"
                  ...
                  >
                    <div className="mb-3">
                      ...
                      <div className="h5 mb-0 text-success">
                        ₱
                        {(parseFloat(restatingBill.total_payable) || 0).toFixed(
                          2
                        )}
                      </div>
                    </div>
